...
namespace UnityEssentials
{
    ...
    public abstract class ChannelReader<T>
    {
        ...
        public virtual Task<T> ReadAsync(CancellationToken cancellationToken = default(CancellationToken))
        {
        ...
        async Task<T> ReadAsyncCore(CancellationToken cancellationToken = default(CancellationToken))
        {
                ...
                lock (parent.items)
                {
                    ...
                    if (parent.items.Count == 0)
                    {
                        if (error == null)
                        {
                            ...
                            {
                                parent.completedTask = Task.CompletedTask;
                            }
                        }
                        else
                        {
                            ...
                            {
                                parent.completedTask = Task.FromException(error);
                            }
                        }
                    ...
                    if (parent.items.Count != 0)
                    {
                        ...
                        if (parent.closed && parent.items.Count == 0)
                        {
                            if (parent.completionError != null)
                            {
                                ...
                                {
                                    parent.completedTask = Task.FromException(parent.completionError);
                                }
                            }
                            else
                            {
            ...
            public override Task<bool> WaitToReadAsync(CancellationToken cancellationToken)
            {
                ...
                lock (parent.items)
                {
                    ...
                    if (parent.closed)
                    {
                        ...
                        {
                            return Task.FromException<bool>(parent.completionError);
                        }
                    }
                    ...
                    return new Task<bool>(this, core.Version);
                }
            }
