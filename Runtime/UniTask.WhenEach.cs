...
namespace UnityEssentials.Threading.Tasks
{
    ...
    {
        public static IUniTaskAsyncEnumerable<WhenEachResult<T>> WhenEach<T>(IEnumerable<Task<T>> tasks)
        {
            return new WhenEachEnumerable<T>(tasks);
        }

        public static IUniTaskAsyncEnumerable<WhenEachResult<T>> WhenEach<T>(params Task<T>[] tasks)
        {
            return new WhenEachEnumerable<T>(tasks);
        }
    }
    ...
    internal sealed class WhenEachEnumerable<T> : IUniTaskAsyncEnumerable<WhenEachResult<T>>
    {
        ...
        public WhenEachEnumerable(IEnumerable<Task<T>> source)
        {
            this.source = source;
        }
        ...
        {
            readonly IEnumerable<Task<T>> source;
            ...
            public Enumerator(IEnumerable<Task<T>> source, CancellationToken cancellationToken)
            {
                this.source = source;
            ...
            static void ConsumeAll(Enumerator self, Task<T>[] array, int length)
            {
