...
namespace UnityEssentials.Threading.Tasks
{
    ...
    public readonly partial struct Task
    {
        ...
        [DebuggerHidden]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Task(IUniTaskSource source, short token)
        {
            this.source = source;
            this.token = token;
        }
        ...
        /// </summary>
        public Task<bool> SuppressCancellationThrow()
        {
            var status = Status;
            if (status == UniTaskStatus.Succeeded) return CompletedTasks.False;
            if (status == UniTaskStatus.Canceled) return CompletedTasks.True;
            return new Task<bool>(new IsCanceledSource(source), token);
        }

#if SUPPORT_VALUETASK

        public static implicit operator System.Threading.Tasks.ValueTask(in Task self)
        {
            if (self.source == null)
            {
                return default;
        ...
        public Task Preserve()
        {
            ...
            {
                return new Task(new MemoizeSource(source), token);
            }
        }

        public Task<AsyncUnit> AsAsyncUnitUniTask()
        {
            ...
            {
                return new Task<AsyncUnit>(asyncUnitSource, this.token);
            }

            return new Task<AsyncUnit>(new AsyncUnitSource(this.source), this.token);
        }
    ...
    public readonly struct Task<T>
    {
        ...
        [DebuggerHidden]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Task(IUniTaskSource<T> source, short token)
        {
            this.source = source;
            this.token = token;
            this.result = default;
        ...
        public Task<T> Preserve()
        {
            ...
            {
                return new Task<T>(new MemoizeSource(source), token);
            }
        }

        public Task AsUniTask()
        {
            if (this.source == null) return Task.CompletedTask;
            ...
            return new Task(this.source, this.token);
        }

        public static implicit operator Task(Task<T> self)
        {
        ...
        public static implicit operator System.Threading.Tasks.ValueTask<T>(in Task<T> self)
        {
            if (self.source == null)
            {
        ...
        /// </summary>
        public Task<(bool IsCanceled, T Result)> SuppressCancellationThrow()
        {
            if (source == null)
            {
                return new Task<(bool IsCanceled, T Result)>((false, result));
            }

            return new Task<(bool, T)>(new IsCanceledSource(source), token);
        }
