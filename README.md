...
public async Task<string> DemoAsync()
    {
        ...
        // .ToTask accepts progress callback(and all options), Progress.Create is a lightweight alternative of IProgress<T>
        var asset3 = await Resources.LoadAsync<TextAsset>("baz").ToTask(Progress.Create<float>(x => Debug.Log(x)));

        // await frame-based operation like a coroutine
        await Task.DelayFrame(100);

        // replacement of yield return new WaitForSeconds/WaitForSecondsRealtime
        await Task.Delay(TimeSpan.FromSeconds(10), ignoreTimeScale: false);

        // yield any playerloop timing(PreUpdate, Update, LateUpdate, etc...)
        await Task.Yield(PlayerLoopTiming.PreLateUpdate);
    ...
    await Task.WaitForEndOfFrame(this); // this is MonoBehaviour
#endif

        // replacement of yield return new WaitForFixedUpdate(same as Task.Yield(PlayerLoopTiming.FixedUpdate))
        await Task.WaitForFixedUpdate();

        // replacement of yield return WaitUntil
        await Task.WaitUntil(() => isActive == false);

        // special helper of WaitUntil
        await Task.WaitUntilValueChanged(this, x => x.isActive);

        // You can await IEnumerator coroutines
        await UnityCalculatePrimes();
        await CalculatePrimes();

        // You can await a standard task
        await System.Threading.Tasks.Task.Run(() => 100);
        ...
        async Task<string> GetTextAsync(UnityWebRequest req)
        {
        ...
        var (google, bing, yahoo) = await Task.WhenAll(task1, task2, task3);

        // shorthand of WhenAll, tuple can await directly
        var (google2, bing2, yahoo2) = await (task1, task2, task3);

        // return async-value.(or you can use `Task`(no result), `TaskVoid`(fire and forget)).
